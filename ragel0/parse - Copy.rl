...
action stmtEnd {
  ...
  if(!mtclParseStmtEndCallback(extraData)) {  //EVAL_WORDS
    runErr=true;
    fbreak;
  }
}
...
action wordEnd {
  ...
  if(mStart<mEnd) { //APPEND_WORD
    ...
    bool r=mtclParseStrCallback(extraData,mStart,mEnd,x);
    free(x);
  ...
  if(!mtclParseWordEndCallback(extraData)) { //APPEND_WORDS
    fbreak;
  } 
}
...
enum {
  PT_WORD,PT_END,PT_PAREN_ERR,PT_ERR
};

#ifdef MTCL_PARSE_TEST
#define mtclParseStrCallback(DATA,A,B,X) 1
#define mtclParseCmdCallback(DATA,A,B,X) 1
#define mtclParseWordEndCallback(DATA) 1
#define mtclParseStmtEndCallback(DATA) 1
#define mtclParseParenErrCallback(D,P)
#define mtclParseErrCallback(D,P)
#define mtclParseRunErrCallback(D,P)
...
void mtclParse(const char *text,void *extraData) {
  %% write data;
  ...
  if(runErr) {
    mtclParseRunErrCallback(extraData,fsm.p);
  } else if(parenErr) {
    DEBUG_MARK("paren err",fsm.p);
    mtclParseParenErrCallback(extraData,prnStart);
  } else if(fsm.cs < %%{ write first_final; }%%) {
    DEBUG_MARK("err",fsm.p);
    mtclParseErrCallback(extraData,fsm.p);
  } 
}
